function [ID,b] = grad_descent_chantelle(I,IPrime,epsilon,nIter)
    % initialize b
    b = zeros(2,1);
...
for i = 1 : nIter
    ...
    gradMatchx = -sum(sum((ID - IPrime).*gradIx));
    gradMatchy = -sum(sum((ID - IPrime).*gradIy));
    
    % update velocity. output = a number
    b(1,1) = b(1,1) - (gradMatchx*epsilon);
    b(2,1) = b(2,1) - (gradMatchy*epsilon);    
end
